...
<script>
	function list(page) {
		location.href = "/product/list?cur_page=" + page
				+ "&keyword=${map.keyword}";
	}
</script>
</head>
<body>
   ...
   <c:if test="${sessionScope.userid != 'admin'}">
	...
	<c:if test="${map.keyword != null}">
		<h1 align="center">${map.keyword}의검색결과</h1>
	</c:if>
	<div class="container">
		<div class="row">
				...
				<div class="col-lg-4 col-md-6 col-sm-12">
					<article class="card-link">
						...
						<a class="card" style="margin: 10px; text-decoration: none;"
							...
							<div>
								<h2 class="card-title"
									style="text-align: center; font-size: 20px; font-weight: bold;">${row.subject}</h2>
								...
								<div class="card-content">
									<span style="font-size: small">${map.memberDao.address(row.userid)}</span>
								</div>
								<div class="card-mini">
									<span style="font-size: small">관심 :
										${map.ldao.love_count(row.write_code)}</span> ∙ <span
										style="font-size: small">조회수 : ${row.see} </span>
								</div>
							</div>
						</a>
					</article>
